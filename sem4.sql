...
-- Вывести учителей с предметами, которые они ведут
SELECT
	t.surname, 
    l.course -- l = lesson
...
-- Вывести всех учителей, даже если они уроки не ведут 
SELECT
	t.*, 
    l.* -- l = lesson
FROM teacher t  -- t = teacher
LEFT JOIN lesson l-- INNER JOIN = JOIN
ON l.teacher_id = t.id;

-- Учителя, которые предметы не ведут 
SELECT
	t.*, 
    l.* -- l = lesson
FROM teacher t  -- t = teacher
LEFT JOIN lesson l-- INNER JOIN = JOIN
ON l.teacher_id = t.id
WHERE l.teacher_id IS NULL; -- True, False

-- Учителя , которые ведут уроки по курсу "Знакомство с веб-технологиями"
SELECT
	t.*, 
    l.* -- l = lesson
FROM teacher t  -- t = teacher
...
-- Выборка данных по пользователю: город, ФИО, название фотки 
...
FROM users u
JOIN `profiles` p ON u.id = p.user_id -- буква ё на англ. раскладке
JOIN media m ON p.photo_id = m.id;

-- Cписок медиафайлов конкретного человека с указаением лайков и выведем ТОП-3 по
-- популярности

SELECT
	CONCAT(u.firstname, " ", u.lastname) AS fullname, -- "Михаил Меркушов"
    m.filename AS media_name,
    m.id,
	COUNT(firstname) AS total_likes
FROM media m 
