...
FROM staff
ORDER BY salary; -- Автоматически идет сортировка по воз-ю, DESC - по убыванию

-- Получим ТОП-5 самых высокооплачиваемых сотрудников в нашей фирме
SELECT 
	id,
    salary,
    CONCAT(firstname," ", lastname) AS fullname --  "Вася" + " " + "Петров"  = "Вася Петров"
...
SELECT 
    ...
    lastname
FROM staff
ORDER BY lastname DESC,firstname DESC, salary;  -- "Я" -> "А"
...
SELECT 
	post,
    ROUND(AVG(salary), 2) AS avg_s, -- до 2 знаков после запятой
    ROUND(SUM(salary))AS sum_s, -- до целого
...
-- Создадим архив по месяцам (сколько файлов было создано в каждом месяце)
SELECT
...
-- Посчитаем количество файлов у каждого юзера
SELECT
	COUNT(id) AS count_media,
